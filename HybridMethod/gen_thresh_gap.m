function [Gs] = gen_thresh_gap(G, shrink, isTransient)

% computation of persistence weights
if isTransient
  W = pers_weights_transient(G);
else
  W = pers_weights_tonal(G);
end
...
GGs = max(Gs, 0); % equivalent: GGs = (Gs.*(Gs>0));
...
if shrink.orth == 1
  ...
  c2 = shrink.center(2);
  cc1 = ceil(MM/2);
  cc2 = ceil(NN/2); % take actual centers of neighborhood
