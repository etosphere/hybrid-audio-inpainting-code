function [Partials, time, padding, L, S, reliableFrameMask, extraFrameNum] = jun_track_partials(y, fs, varargin)
% ********************************************************************* %
...
optargs = {N HopFactor OverSample G_g Q delta zetaF zetaA zetaFS mask};
optargs(1:numvarargs) = varargin;
[N, HopFactor, OverSample, G_g, Q, delta, zetaF, zetaA, zetaFS, mask] = optargs{:};
...
maskPad = [true(padding(1),1); mask; true(padding(2),1)];
...
for m = 1:M
    ...
    reliableNum = sum(maskPad(time(m)-n_center:time(m)+Ndft-1-n_center));
    if (reliableNum >= Ndft/4*3)
      [Alpha, num_peaks, S(:,m)] = jun_ddm(yst, Q, Rddm, G_g, win, winD, ...
        p, pD, centering, Ndft, ft_mat, omega);
      if (reliableNum < Ndft)
        extraFrameNum = extraFrameNum + 1;
      end
    else
      Alpha = 0;
      num_peaks = 0;
      S(:, m) = fft(win.*yst, Ndft);
      reliableFrameMask(m) = false;
    end
    ...
    if m > 1
        ...
        if num_peaks > 0 && num_peaks_last > 0
            ...
            if Q == 2
              % f'(+H/2) [k-1]
              mFS1 = fs/(2*pi) * imag(2 * Alpha_last(3,:));
              % f'(-H/2) [k]
              mFS2 = fs/(2*pi) * imag(2 * Alpha(3,:));
            elseif Q == 3
              mFS1 = fs/(2*pi) * imag(([2, 3] .* pD(n_midpoint(2),1:2)) * Alpha_last(3:end,:));
              mFS2 = fs/(2*pi) * imag(([2, 3] .* pD(n_midpoint(1),1:2)) * Alpha(3:end,:));
            end
            ...
            % change delta F to erb scale
            DeltaF = bsxfun(@minus, freq_to_erb(mF1'), freq_to_erb(mF2));

            % DeltaFS = bsxfun(@minus, mFS1', mFS2);
            DeltaFS = bsxfun(@minus, mFS1' ./ freq_to_erb(mF1'), mFS2 ./ freq_to_erb(mF2));

            varFS = -zetaFS^2*log((delta-1)/(delta-2));
            
            % USEFUL COST
            % eq. (8)
            % A_useful = 1-exp(-DeltaF.^2/varF - DeltaA.^2/varA);
            A_useful = 1-exp(-DeltaF.^2/varF - DeltaA.^2/varA - DeltaFS.^2/varFS);
            % SPURIOUS COST
            % eq. (9)
...
extraFrameNum = ceil(extraFrameNum / 2);
